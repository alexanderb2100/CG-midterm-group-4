...
void keyboard(GameObject::Sptr obj1) {
	...
	if (glfwGetKey(Application::Get().GetWindow(), GLFW_KEY_D) == GLFW_PRESS)
		moveX += 0.1f;
	glm::vec3 move(moveX, moveY, 0.0f);
...
void RenderLayer::OnRender(const Framebuffer::Sptr& prevLayer)
{
	...
	app.CurrentScene()->Components().Each<RenderComponent>([&](const RenderComponent::Sptr& renderable) {
		...
		GameObject::Sptr win = app.CurrentScene()->FindObjectByName("Specular Object");
		GameObject::Sptr lose = app.CurrentScene()->FindObjectByName("Foliage Sphere");
		keyboard(player);
		if (player->GetPosition().x <= win->GetPosition().x + 1.5f && player->GetPosition().x >= win->GetPosition().x - 1.5f) {
			if (player->GetPosition().y >= win->GetPosition().y - 1.5f && player->GetPosition().y <= win->GetPosition().y + 1.5f)
				std::cout << "Win\n";
			player->SetPostion(glm::vec3(1.5f, 0.0f, 1.0f));
		}

		if (player->GetPosition().x <= lose->GetPosition().x + 1.5f && player->GetPosition().x >= lose->GetPosition().x - 1.5f) {
			if (player->GetPosition().y >= lose->GetPosition().y - 1.5f && player->GetPosition().y <= lose->GetPosition().y + 1.5f)
				std::cout << "Lose\n";
			player->SetPostion(glm::vec3(1.5f, 0.0f, 1.0f));
		}
